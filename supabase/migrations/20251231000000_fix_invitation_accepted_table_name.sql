-- =====================================================
-- Fix: chat_room_participants → chat_participants
-- Fix: chat_room_id → room_id
-- =====================================================

-- 초대 수락 시 자동 멤버 추가 함수 (수정)
...
BEGIN
  ...
  IF NEW.status = 'accepted' AND OLD.status <> 'accepted' THEN
    ...
    IF v_target_type = 'project' THEN
      ...
      -- 프로젝트 채팅방에 추가 (수정: chat_participants, room_id)
    ...
    ELSIF v_target_type = 'collaboration' THEN
      ...
      -- 협업 채팅방에 추가 (수정: chat_participants, room_id)
...
DO $$
BEGIN
  RAISE NOTICE 'handle_invitation_accepted 함수 수정 완료: chat_room_participants → chat_participants, chat_room_id → room_id';
END $$;
