-- ============================================================================
-- Chat Notification: message vs group_message 타입 분기
-- - 참여자 3명 이상: group_message
-- - 참여자 2명 이하: message
-- ============================================================================
...
BEGIN
    ...
    -- 참여자 3명 이상이면 group_message, 아니면 message
    ...
    LOOP
        ...
        -- 알림 타입에 따라 설정 확인 (message 또는 group_message)
        IF check_notification_enabled(participant.user_id, notification_type) THEN
...
-- 마이그레이션 적용 확인용 코멘트
COMMENT ON FUNCTION public.handle_new_chat_message() IS '채팅 메시지 생성 시 알림 발송 - 그룹(3명+)은 group_message, 개인은 message 타입';
